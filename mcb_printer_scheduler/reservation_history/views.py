...
def view_create_invoice(request, res_id):
    ...
    if request.method == 'POST':
        ...
        if frm.is_valid():
            ...
            frm_data['note'] = mark_safe("%s<br />\n%d sq. feet" % (frm_data['note'], frm_data['sq_feet']))
        ...
        note_array = [reservation.lab_name, reservation.note, reservation.poster_tube_details]
        note_array = filter(None, note_array)
        note_string = '\n'.join(note_array)
        initial_form_data = {'note': note_string,
                             'last_update': reservation.last_update,
                             'sq_feet': 0.0}
...
def view_save_invoice(request, lu):
    ...
    return download_invoice(request, invoice_no, filename)
...
def download_invoice(request, invoice_no, filename):
    ...
    url = 'http://localhost/poster-printer/history/display-invoice/%s/' % (invoice_no)
