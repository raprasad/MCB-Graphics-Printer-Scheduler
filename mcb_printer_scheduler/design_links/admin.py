...
from design_links.models import Organization, DesignLinkBase, DesignImage, DesignImageNonWeb, DesignLink
...
class DesignImageNonWebAdminInline(admin.TabularInline):
    model = DesignImageNonWeb
    form = DesignImageAdminForm
    #readonly_fields = ('view_link',)
    list_editable = ('sort_field',)
    fields = ('name', 'sort_field', 'description', 'image_file',)
    extra=0

class OrganizationAdmin(admin.ModelAdmin):
    save_on_top = True
    inlines = (DesignImageAdminInline, DesignImageNonWebAdminInline, DesignLinkAdminInline)
...
class DesignImageNonWebAdmin(admin.ModelAdmin):
    #form = DesignImageAdminForm
    save_on_top = True
    list_display= ('organization', 'name', 'sort_field','created', 'last_update', )
    search_fields = ( 'name', 'description')
    list_editable = ('sort_field',)
    
    readonly_fields = ('created', 'last_update', 'link_type',  )
    fieldsets = [
        ('', { 'fields':  [  'organization', 'name', 'description',  'image_file',  'link_type', 'sort_field', 'created', 'last_update' ]}), ]
admin.site.register(DesignImageNonWeb, DesignImageNonWebAdmin)
